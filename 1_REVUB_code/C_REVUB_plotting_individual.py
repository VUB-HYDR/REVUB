...
if Figure2_on == 1:
    ...
    ax2.loglog(fft_CONV_freq*hrs_day*secs_hr, fft_CONV_amp, color = colour_CONV)
    if calibration_only == 0:
        ax2.loglog(fft_BAL_freq*hrs_day*secs_hr, fft_BAL_amp, color = colour_BAL)
        if STOR_break[plot_HPP] == 0 and option_storage == 1:
            ax2.loglog(fft_STOR_freq*hrs_day*secs_hr, fft_STOR_amp, color = colour_STOR, linestyle = '--')
            ax2.legend(['CONV', 'BAL', 'STOR'])
        else:
            ax2.legend(['CONV', 'BAL'])
    else:
        ax1.legend(['CONV'])
    ax2.set_xlim([fft_CONV_freq[1]*hrs_day*secs_hr, hrs_day/2])
    ax2.set_xticks([1/np.sum(np.mean(days_year,1)), 1])
    ax2.set_xticklabels(['1 year', '1 day'])
    ax2.set_ylim([1e-7, 2])
    ax2.set_xlabel('Signal frequency (day$^{-1}$)')
    ax2.set_ylabel('Signal strength')
    ax2.set_title('frequency spectrum of hydraulic head', fontsize = 'small')
...
if d_min[plot_HPP] != 1 and calibration_only == 0:
    ...
    # [figure] histogram of flexibility regime by month in BAL regime
    if Figure9_on == 1:
