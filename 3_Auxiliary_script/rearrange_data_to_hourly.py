...
data = pd.read_excel (filename_data, sheet_name = 'monthly_series', header = None)
...
for q in range(series_number):
    ...
    # [parse] formatted data into input file
    with pd.ExcelWriter(filename_data, engine = "openpyxl", mode = "a", if_sheet_exists = "replace") as writer:
        pd.DataFrame(output_hourly_byyear[q,:,:]).to_excel(writer, sheet_name = "".join(char for char in header[q] if char.isalnum() or char == " ")[:31], index = False, header = False)

print('done with monthly-to-full-hourly series (format 1)')
...
for q in range(series_number):
    ...
    for y in range(no_years):
        ...
        if (year/4).is_integer():
            for m in range(months_yr):
                output_hourly_byyear[q, int(positions_month_leap[m]) : int(positions_month_leap[m+1]), y] = np.tile(data_for_conversion[m*dayhrs : (m+1)*dayhrs, q], days_leap[m])
        else:
            for m in range(months_yr):
                output_hourly_byyear[q, int(positions_month[m]) : int(positions_month[m+1]), y] = np.tile(data_for_conversion[m*dayhrs : (m+1)*dayhrs, q], days[m])
    
    # [parse] formatted data into input file
    with pd.ExcelWriter(filename_data, engine = "openpyxl", mode = "a", if_sheet_exists = "replace") as writer:
        pd.DataFrame(output_hourly_byyear[q,:,:]).to_excel(writer, sheet_name = "".join(char for char in header[q] if char.isalnum() or char == " ")[:31], index = False, header = False)

print('done with hourly-bymonth-to-full-hourly series (format 2)')
